...
public class IndexController {
	...
	public String loginProc(@RequestParam Map<String, String> map, Model model,
		...
		}else {//실패 : 다시 로그인 하러가기 .((미션)) alert 메시지 띄우기 "로그인 정보가올바르지 않습니다.!"
	...
	public String logout2(HttpServletRequest request) {
		...
		session.invalidate();
//서버가 JESSIONID는 새로 부여해주지만@SessionAttributes로 설정된 애트리뷰트 값은 남아있다.
		return "redirect:/";
	}
...
//				 session.removeAttribute("");	//- JSESSIONID 는 변하지 않고 값 삭제
