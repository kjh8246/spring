 ...
 */
@Controller			//@Conponent 대신에 열할을 표시하는 어노테이션
public class HomeController {
	
	private static final Logger logger = LoggerFactory.getLogger(HomeController.class);
	
	/**
	 * @RequestMapping : url요청을 처리할 메소드를 지정하는 어노데이션
	...
	public void search(String name, int age) {	
		...
		//int값은 파라미터는 기본이 string이므로 null일때 , int변환못하고 오류
	...
	public String order(Order order) {
		//메소드의 매개변수는 파라미터가 여러개일때 model 객체를 사용합니다.
		logger.info("[My]"+order);		//확인
		return "home";	// url 은 order, 화면은 home.jsp
	}

	@RequestMapping(value="map")	//method속성은 기본값RequestMethod.GET
	public String map(@RequestParam Map<String,Object> param) {
		//@RequestParam : 이것은 요청 파라미터다
		//클래스 정의없이 Map을 model객체로 사용한 경우
		logger.info("[My]"+param);
		return "redirect:/";	//url이 context ~~~~/app/ 로 새로운 요청으로 응답
		//home : http://localhost:8085/app/map?idx=23&cnt=44&title=momo&page=11
		//redirect : http://localhost:8085/app/
  	}
	...
	public String redirect(Model model) {
		...
		//실행 했을때 url을 확인합니다 : http://localhost:8085/app/?idx=99
	}
